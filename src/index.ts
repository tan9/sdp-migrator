...
git.checkIsRepo().then((isRepo: boolean) => {
    if (!isRepo) {
...
await replaceInFile({
    ...
    .then(commitReplacedResults(`將 Parent POM 更新為 ${targetSdpVersion}`))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("將 Shared Libraries 對應到 SDP 版本"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("移除 pom.xml 的 build.outputDirectory 設定"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("移除 pom.xml 的 maven-compiler-plugin 設定"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("移除 pom.xml 中的空 build 設定"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("修正 pom.xml repository 的網址"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults(`移除 beans config 裡的 component-scan 設定`))
    .catch((error) => {
...
await glob("**/src/**/persistence-*.xml")
    .then(deleteFilesAndCommit("刪除 persistence-*.xml"))
    .catch((error) => exit(MIGRATE_ERROR, `刪除 persistence-*.xml 時發生錯誤: ${error}`));

await glob("**/src/**/mvc-config.xml")
    .then(deleteFilesAndCommit("刪除 mvc-config.xml"))
    .catch((error) => exit(MIGRATE_ERROR, `刪除 mvc-config.xml 時發生錯誤: ${error}`));

await glob("**/src/**/logback.xml")
    .then(deleteFilesAndCommit("刪除 logback.xml"))
    .catch((error) => exit(MIGRATE_ERROR, `刪除 logback.xml 時發生錯誤: ${error}`));
...
await Promise.resolve(
    ...
    .then(moveFilesAndCommit("異動 tiles-front.xml 位置"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("將 sun.misc.BASE64 改寫為 java.util.Base64"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults("改用 Quartz 2 API"))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults(`${projectId}-scheduler-beans-config 改用新版 Spring Quartz 整合機制`))
    .catch((error) => {
...
await replaceInFile({
    ...
    .then(commitReplacedResults(`修正 jsp import 結尾包含 ; 的問題`))
    .catch((error) => {
